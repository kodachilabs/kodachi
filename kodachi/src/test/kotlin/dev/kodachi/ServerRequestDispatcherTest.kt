...
import dev.kodachi.internal.ServerRequestDispatcher
    ...
    fun `an overriding handler can fulfil a request the default declines`() {
        val handler = object : ServerRequestHandler() {
            override suspend fun onAttestation(
                params: dev.kodachi.protocol.AttestationGenerateParams,
            ) = dev.kodachi.protocol.AttestationGenerateResponse(token = "tok-123")
        }
