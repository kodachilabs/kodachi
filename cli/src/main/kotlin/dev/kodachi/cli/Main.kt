...
import dev.kodachi.protocol.GetAccountParams
...
kodachi — exercise the Kotlin Codex SDK from a terminal

USAGE
  kodachi doctor                       Check the setup. Spends no model quota.
  kodachi login                        Store an API key (read from stdin or ${'$'}OPENAI_API_KEY).
  kodachi whoami                       Show the stored account and rate limits.
  kodachi models                       List models this account can use.
  kodachi chat <prompt>                Run one turn and stream it.
  kodachi goal <objective>             Run a goal across as many turns as it takes.
  kodachi exec <prompt>                Run one turn with writes allowed, approving everything.
...
AUTH
  ...
  visible to other processes and lands in shell history. `kodachi login` stores it in the
...
private fun open(options: Options): Codex {
    val env = buildMap {
        if (options.isolated) {
            val home = File(System.getProperty("java.io.tmpdir"), "kodachi-cli-home-${System.nanoTime()}")
            home.mkdirs()
...
private suspend fun doctor(options: Options) {
    println("kodachi ${ProtocolInfo.CODEX_VERSION} protocol layer")
    ...
    open(options).use { codex ->
        ...
        account.fold(
            ...
            onFailure = { println("account          none or unreadable — run `kodachi login`") },
        )
        ...
        println()
        println("Ready. Try:  kodachi chat \"say hello\"")
    }
}
    ...
    if (key.isBlank()) {
        System.err.println(
            ...
            No API key found.
              ...
              export OPENAI_API_KEY=...   &&  kodachi login
              printenv OPENAI_API_KEY | kodachi login
    ...
    open(options).use { codex ->
        ...
        println(
            "NOTE: the server stores a key without validating it, so this does not prove the " +
                "key works. Run `kodachi chat \"hi\"` to find out.",
        )
    }
}
...
private suspend fun whoami(options: Options) {
    open(options).use { codex ->
        runCatching { codex.account.read(GetAccountParams()) }.fold(
            ...
            onFailure = { println("no account readable — run `kodachi login`") },
        )
...
private suspend fun chat(options: Options, writable: Boolean) {
    val prompt = options.positional.joinToString(" ").ifBlank {
        System.err.println("kodachi: nothing to send. Try: kodachi chat \"say hello\"")
        exitProcess(2)
    }
...
private suspend fun goal(options: Options) {
    val objective = options.positional.joinToString(" ").ifBlank {
        System.err.println("kodachi: no objective. Try: kodachi goal \"make the tests pass\"")
        exitProcess(2)
    }
...
private suspend fun stream(
    flow: kotlinx.coroutines.flow.Flow<dev.kodachi.protocol.CodexNotification>,
    quiet: Boolean,
) {
    var turns = 0
    flow.collect { event ->
